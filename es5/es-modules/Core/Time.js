...
var Time = /** @class */ (function (_super) {
    ...
    Time.prototype.getBoundaryTicks = function (tickPositions, unitRange, visibleMin, visibleMax) {
        var boundaryTicks = {};
        // No boundary ticks for year range.
        if (unitRange === timeUnits.year) {
            return boundaryTicks;
        }
        var hasVisibleRange = defined(visibleMin) && defined(visibleMax), needsHourBoundary = unitRange < timeUnits.hour, needsDayBoundary = unitRange < timeUnits.day, needsMonthBoundary = unitRange < timeUnits.month, tickAlignment = (unitRange >= timeUnits.hour ?
        ...
        if (tickPositions.length < 10000) {
            var isFirstVisibleTick = true;
            for (var i = 0; i < tickPositions.length; i++) {
                var t = tickPositions[i];
                ...
                var _a = this.toParts(t), // Unused 'year' var
                month = _a[1], day = _a[2], hours = _a[3], minutes = _a[4], seconds = _a[5], milliseconds = _a[6];
                var isMidnight = !hours && !minutes && !seconds && !milliseconds;
    ...
    Time.prototype.getTimeTicks = function (normalizedInterval, min, max, startOfWeek) {
        var time = this, tickPositions = [], _a = normalizedInterval.count, count = _a === void 0 ? 1 : _a, unitRange = normalizedInterval.unitRange, visibleMin = min, visibleMax = max;
