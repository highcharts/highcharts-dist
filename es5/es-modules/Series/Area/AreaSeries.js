...
var AreaSeries = /** @class */ (function (_super) {
    ...
    AreaSeries.prototype.getGraphPath = function (points) {
        ...
        addDummyPoints = function (i, otherI, side) {
            var point = points[i], otherPoint = points[otherI], stackedValues = stacking && (stacks[point.x].points[seriesIndex]), nullVal = point[side + 'Null'] || 0, cliffVal = point[side + 'Cliff'] || 0;
    ...
    AreaSeries.prototype.getStackPoints = function (points) {
        var series = this, segment = [], keys = [], xAxis = this.xAxis, yAxis = this.yAxis, stack = yAxis.stacking.stacks[this.stackKey], pointMap = {}, yAxisSeries = yAxis.series, seriesLength = yAxisSeries.length, upOrDown = yAxis.options.reversedStacks ? 1 : -1, seriesIndex = yAxisSeries.indexOf(series), translatedThreshold = yAxis.getThreshold(series.options.threshold || 0);
        points = points || this.points;
        if (this.options.stacking) {
            ...
            keys.forEach(function (x, idx) {
                ...
                if (pointMap[x] && !pointMap[x].isNull) {
                    ...
                    y || (y = 0);
                    var plotY = yAxis.positiveValuesOnly && y <= 0 ?
                        translatedThreshold :
