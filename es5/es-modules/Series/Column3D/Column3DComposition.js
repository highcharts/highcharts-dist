...
function compose(SeriesClass, StackItemClass) {
    if (pushUnique(composed, 'Column3D')) {
        var seriesProto = SeriesClass.prototype, stackItemProto = StackItemClass.prototype, _a = SeriesClass.types, ColumnSeriesClass = _a.column, ColumnRangeSeriesClass = _a.columnRange;
...
function wrapSeriesAlignDataLabel(proceed, point, _dataLabel, options, alignTo) {
    ...
    if (chart.is3d() &&
        this.is('column')) {
        var series = this, seriesOptions = series.options, inside = pick(options.inside, !!series.options.stacking), options3d = chart.options.chart.options3d, xOffset = (point.pointWidth || 0) / 2;
        var dLPosition = {
            x: alignTo.x + xOffset,
            y: alignTo.y,
