...
function hasExtremes(series, checkX) {
    var options = series.options, threshold = pick(options.boostThreshold, Number.MAX_VALUE);
    if (threshold === 0) {
        return false;
    }
    var dataLength = series.dataTable.getModified().rowCount, xAxis = series.xAxis && series.xAxis.options, yAxis = series.yAxis && series.yAxis.options, colorAxis = series.colorAxis && series.colorAxis.options;
...
var getSeriesBoosting = function (series, data) {
    var _a;
    var options = series.options, forceCrop = series.forceCrop, chart = series.chart, threshold = pick(options.boostThreshold, Number.MAX_VALUE);
    // Return early if either will be grouped or boost is disabled.
    if (forceCrop || threshold === 0) {
        return false;
    }
    return isChartSeriesBoosting(chart) || ((_a = data === null || data === void 0 ? void 0 : data.length) !== null && _a !== void 0 ? _a : 0) >= threshold;
};
/**
...
function onSeriesHide() {
    var boost = this.boost, chartBoost = this.chart.boost, sharedMarkerGroup = chartBoost === null || chartBoost === void 0 ? void 0 : chartBoost.markerGroup;
...
function scatterProcessData(force) {
    ...
    var processedXData = [], processedYData = [], xRangeNeeded = !(isNumber(xExtremes.max) || isNumber(xExtremes.min)), yRangeNeeded = !(isNumber(yExtremes.max) || isNumber(yExtremes.min));
...
function seriesRenderCanvas() {
    ...
    /** @internal */
    var boostOptions = renderer.settings, chunkSize = (isNumber(boostOptions.chunkSize) && boostOptions.chunkSize > 0 ?
        boostOptions.chunkSize :
        CHUNK_SIZE), doneProcessing = function () {
