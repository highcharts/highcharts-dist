...
function getFreeIndexes(numberOfColumns, seriesBuilders) {
    ...
    var freeIndexes = new Array(numberOfColumns).fill(true), freeIndexValues = [];
        ...
        function performFetch(initialFetch) {
            ...
            function request(url, done, tp) {
                if (!url ||
                    !/^(http|\/|\.\/|\.\.\/)/.test(url)) {
                    if (url) {
                        error("Invalid URL: ".concat(url), false, chart);
                    }
                    return false;
                }
                if (initialFetch) {
                ...
                ajax({
                    ...
                    error: function (xhr, e) {
                        ...
                        return error("Request failed - ".concat(xhr.status, " \n") +
    ...
    Data.prototype.getDataTable = function () {
        return new DataTableCore({
            columns: Object.values(this.columns || [])
                ...
                var column = dtColumn.slice(), columnId = column.shift();
