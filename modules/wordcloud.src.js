...
(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		define("highcharts/modules/wordcloud", ["highcharts/highcharts"], function (amd1) {return factory(amd1["SeriesRegistry"],amd1);});
	else if(typeof exports === 'object')
		exports["highcharts/modules/wordcloud"] = factory(root["_Highcharts"]["SeriesRegistry"], root["_Highcharts"]);
	else
...
function getAxesFromPolygon(polygon) {
    ...
    if (!axes.length) {
        ...
        points.reduce((p1, p2) => {
            ...
            if (!(0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.find)(axes, (existing) => existing[0] === axis[0] &&
                existing[1] === axis[1])) {
                axes.push(axis);
            }
...
function isPolygonsColliding(polygon1, polygon2) {
    ...
    const axes1 = getAxesFromPolygon(polygon1), axes2 = getAxesFromPolygon(polygon2), axes = axes1.concat(axes2), overlappingOnAllAxes = !(0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.find)(axes, (axis) => isPolygonsOverlappingOnAxis(axis, polygon1, polygon2));
...
function intersectionTesting(point, options) {
    ...
    rect = point.rect = (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.extend)({}, rectangle);
    let attempt = 1, delta = {
        x: 0,
        y: 0
    };
...
function correctFloat(number, precision) {
    const p = (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.isNumber)(precision) ? precision : 14, magnitude = Math.pow(10, p);
...
class WordcloudSeries extends ColumnSeries {
    ...
    deriveFontSize(relativeWeight, maxFontSize, minFontSize) {
        const weight = (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.isNumber)(relativeWeight) ? relativeWeight : 0, max = (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.isNumber)(maxFontSize) ? maxFontSize : 1, min = (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.isNumber)(minFontSize) ? minFontSize : 1;
    ...
    drawPoints() {
        ...
        for (const point of points) {
            const relativeWeight = 1 / maxWeight * point.weight, fontSize = series.deriveFontSize(relativeWeight, options.maxFontSize, options.minFontSize), css = (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.extend)({
        ...
        for (const point of points) {
            const relativeWeight = 1 / maxWeight * point.weight, fontSize = series.deriveFontSize(relativeWeight, options.maxFontSize, options.minFontSize), css = (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.extend)({
                ...
                placed: placed,
                rotation: rotation
            }), attr = (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.extend)(series.pointAttribs(point, (point.selected && 'select')), {
                align: 'center',
                ...
                y: placement.y,
                text: point.name,
                rotation: (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.isNumber)(placement.rotation) ?
                    placement.rotation :
                    void 0
