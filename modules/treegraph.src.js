...
class TreegraphSeries extends TreemapSeries {
     ...
     */
    getLayoutModifiers() {
        const chart = this.chart, series = this, plotSizeX = chart.plotSizeX, plotSizeY = chart.plotSizeY, linkWidth = series.options.link?.lineWidth || 0, columnCount = (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.arrayMax)(this.points.map((p) => p.node.xPosition));
        ...
        this.points.forEach((point) => {
            ...
            const node = point.node, level = series.mapOptionsToLevel[point.node.level] || {}, markerOptions = (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.merge)(this.options.marker, level.marker, point.options.marker), nodeWidth = markerOptions.width ?? TreegraphSeries_getNodeWidth(this, columnCount), radius = (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.relativeLength)(markerOptions.radius || 0, Math.min(plotSizeX, plotSizeY)), lineWidth = Math.max(markerOptions.lineWidth || 0, linkWidth), symbol = markerOptions.symbol, nodeSizeY = (symbol === 'circle' || !markerOptions.height) ?
                radius * 2 :
                (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.relativeLength)(markerOptions.height, plotSizeY), nodeSizeX = symbol === 'circle' || !nodeWidth ?
                radius * 2 :
                (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.relativeLength)(nodeWidth, plotSizeX);
    ...
    getLinks() {
        ...
        this.data.forEach((point) => {
            ...
            if (point.node.parent) {
                const pointOptions = (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.merge)(levelOptions, point.options);
    ...
    drawNodeLabels(points) {
        ...
        for (const point of points) {
            ...
            if (point.shapeArgs &&
                ...
                if (!(0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.splat)(series.options.dataLabels)[0].style?.width) {
                    css.width = `${width}px`;
                }
    ...
    translateNode(point) {
        ...
        { ax, bx, ay, by } = this.layoutModifier, x = this.isCartesian ?
            ...
            ax * node.xPosition + bx, y = ay * node.yPosition + by, level = this.mapOptionsToLevel[node.level] || {}, markerOptions = (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.merge)(this.options.marker, level.marker, point.options.marker), symbol = markerOptions.symbol, height = node.nodeSizeY, width = node.nodeSizeX, reversed = this.options.reversed, nodeX = node.x = (chart.inverted ?
            plotSizeX - width / 2 - x :
