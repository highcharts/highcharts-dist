...
const { doc: DraggableChart_doc } = (highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_default());
...
function addDragDropEvents(chart) {
    ...
    if (isChartDraggable(chart)) {
        ...
        chart.unbindDragDropMouseUp = DraggableChart_addEvents(DraggableChart_doc, ['mouseup', 'touchend'], (e) => {
            mouseUp(DraggableChart_getNormalizedEvent(e, chart), chart);
        }, {
            passive: false
        });
...
function pointShowDragHandles() {
    ...
    // for it.
    for (const key of Object.keys(dragDropProps)) {
        const val = dragDropProps[key], handleOptions = merge(DraggablePoints_DragDropDefaults.dragHandle, val.handleOptions, options.dragHandle), handleAttrs = {
