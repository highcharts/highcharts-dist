...
class SolidGaugeSeries extends GaugeSeries {
    ...
    drawPoints() {
        const series = this, yAxis = series.yAxis, center = yAxis.center, options = series.options, renderer = series.chart.renderer, overshoot = options.overshoot, rounded = options.rounded, borderRadius = optionsToObject(rounded ? '50%' : (options.borderRadius ??
            yAxis.pane.options.borderRadius)).radius, overshootVal = (0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.isNumber)(overshoot) ?
        ...
        for (const point of series.points) {
            ...
            if (!point.isNull) { // Condition like in pie chart
                const paneInnerSize = yAxis.pane.options.innerSize, radius = (((0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.relativeLength)(point.options.radius ??
                    options.radius ??
                    '100%', center[2] / 2))), innerRadius = Math.min(((0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.relativeLength)(point.options.innerRadius ??
                    options.innerRadius ??
                    ((0,highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_.isNumber)(paneInnerSize) ?
                        paneInnerSize / 2 :
                        paneInnerSize) ??
