...
/**
 * @license Highmaps JS v12.5.0-modified (2026-02-15)
...
class MapSeries extends ScatterSeries {
    ...
    pointAttribs(point, state) {
        ...
        if (state) {
            const stateOptions = MapSeries_merge(this.options.states?.[state], point.options.states?.[state] || {}), stateStrokeWidth = this.getStrokeWidth(stateOptions);
...
const { addEvent: BubbleSeries_addEvent, arrayMax: BubbleSeries_arrayMax, arrayMin: BubbleSeries_arrayMin, clamp: BubbleSeries_clamp, defined: BubbleSeries_defined, extend: BubbleSeries_extend, isNumber: BubbleSeries_isNumber, merge: BubbleSeries_merge, pick: BubbleSeries_pick, pushUnique: BubbleSeries_pushUnique } = (highcharts_commonjs_highcharts_commonjs2_highcharts_root_Highcharts_default());
/* *
 *
 *  Functions
 *
...
class HeatmapSeries extends HeatmapSeries_ScatterSeries {
    ...
    pointAttribs(point, state) {
        ...
        if (state && state !== 'normal') {
            const stateOptions = HeatmapSeries_merge(seriesOptions.states?.[state], seriesOptions.marker?.states?.[state], point?.options.marker?.states?.[state] || {});
            attr.fill =
                stateOptions.color ||
