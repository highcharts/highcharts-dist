...
const Loader_H = (external_highcharts_src_js_default_default());
const addedFiles = [];
const loaded = [];
...
['chart', 'ganttChart', 'mapChart', 'stockChart'].forEach((factory) => {
    Loader_H[factory] = async function (container, options, callback) {
        guessRoot();
        ...
        return new Loader_H[constructorName](container, options, callback);
    };
});
        ...
        Object.entries(opts).forEach(([key, value]) => {
            ...
            fullKeys.forEach((fullKey) => {
                if (fullKey in Autoload_DependencyMapping) {
                    Autoload_DependencyMapping[fullKey].forEach((file) => pushUnique(files, file));
                }
            });
    ...
    if (options.annotations) {
        splat(options.annotations).forEach((annotation) => {
            ((annotation.type && (Autoload_DependencyMapping[`annotations.${annotation.type}`] ||
                Autoload_DependencyMapping['annotations.type'])) || []).forEach((file) => pushUnique(files, file));
        });
    }
