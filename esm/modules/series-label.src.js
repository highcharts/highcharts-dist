...
function checkClearPoint(series, x, y, bBox, checkDistance) {
    const chart = series.chart, seriesLabelOptions = series.options.label || {}, onArea = (0,external_highcharts_src_js_default_namespaceObject.pick)(seriesLabelOptions.onArea, !!series.area), findDistanceToOthers = (onArea || seriesLabelOptions.connectorAllowed), leastDistance = 16, boxesToAvoid = chart.boxesToAvoid;
...
function compose(ChartClass, SVGRendererClass) {
    if ((0,external_highcharts_src_js_default_namespaceObject.pushUnique)(composed, 'SeriesLabel')) {
        // Leave both events, we handle animation differently (#9815)
        (0,external_highcharts_src_js_default_namespaceObject.addEvent)(ChartClass, 'load', onChartRedraw);
...
function drawSeriesLabels(chart) {
    ...
    chart.series.forEach(function (series) {
        ...
        const colorClass = ('highcharts-color-' + (0,external_highcharts_src_js_default_namespaceObject.pick)(series.colorIndex, 'none')), isNew = !series.labelBySeries, minFontSize = labelOptions.minFontSize, maxFontSize = labelOptions.maxFontSize, inverted = chart.inverted, paneLeft = (inverted ? series.yAxis.pos : series.xAxis.pos), paneTop = (inverted ? series.xAxis.pos : series.yAxis.pos), paneWidth = chart.inverted ? series.yAxis.len : series.xAxis.len, paneHeight = chart.inverted ? series.xAxis.len : series.yAxis.len, points = series.interpolatedPoints, onArea = (0,external_highcharts_src_js_default_namespaceObject.pick)(labelOptions.onArea, !!series.area), results = [], xData = series.getColumn('x');
        ...
        function insidePane(x, y, bBox) {
            const leftBound = Math.max(paneLeft, (0,external_highcharts_src_js_default_namespaceObject.pick)(areaMin, -Infinity)), rightBound = Math.min(paneLeft + paneWidth, (0,external_highcharts_src_js_default_namespaceObject.pick)(areaMax, Infinity));
            return (x > leftBound &&
        ...
        if (series.visible && !series.boosted && points) {
            if (!label) {
                ...
                if (!chart.renderer.styledMode) {
                    ...
                    label.css((0,external_highcharts_src_js_default_namespaceObject.extend)({
                        color: onArea ?
            ...
            if (results.length) {
                ...
                if (dist && series.labelBySeries) {
                    ...
                    series.labelBySeries
                        .attr((0,external_highcharts_src_js_default_namespaceObject.extend)(attr, {
    ...
    (0,external_highcharts_src_js_default_namespaceObject.fireEvent)(chart, 'afterDrawSeriesLabels');
...
function getPointsOnGraph(series) {
    ...
    if (series.getPointSpline &&
        ...
        for (i = 0; i < len; i += 1) {
            ...
            if ((0,external_highcharts_src_js_default_namespaceObject.isNumber)(plotX) && (0,external_highcharts_src_js_default_namespaceObject.isNumber)(plotY)) {
                const ctlPoint = {
                    plotX,
                    plotY,
                ...
                if (onArea) {
                    ...
                    if (inverted) {
                        ctlPoint.chartCenterX = paneLeft + paneWidth - ((plotHigh ? plotHigh : point.plotY || 0) +
                            (0,external_highcharts_src_js_default_namespaceObject.pick)(point.yBottom, translatedThreshold)) / 2;
                    }
                    else {
...
function onChartRedraw(e) {
    if (this.renderer) {
        ...
        if (chart.seriesLabelTimer) {
            (0,external_highcharts_src_js_default_namespaceObject.internalClearTimeout)(chart.seriesLabelTimer);
        }
        ...
        chart.seriesLabelTimer = (0,external_highcharts_src_js_default_namespaceObject.syncTimeout)(function () {
