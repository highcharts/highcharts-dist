...
class DependencyWheelPoint extends SankeyPoint {
    ...
    getDataLabelPath(label) {
        ...
        if (!point.dataLabelPath) {
            ...
            (0,external_highcharts_src_js_default_namespaceObject.wrap)(label, 'destroy', function (proceed) {
                if (point.dataLabelPath) {
        ...
        point.dataLabelPath = renderer
            ...
            x: shapeArgs.x,
            y: shapeArgs.y,
            r: ((shapeArgs.r || 0) + (0,external_highcharts_src_js_default_namespaceObject.pInt)(label.options?.distance || 0)),
...
class DependencyWheelSeries extends SankeySeries {
    ...
    animate(init) {
        const series = this;
        if (!init) {
            const duration = (0,external_highcharts_src_js_default_namespaceObject.animObject)(series.options.animation).duration, step = (duration / 2) / series.nodes.length;
            let i = 0;
        ...
        for (const node of this.nodeColumns[0]) {
            ...
            if (node.sum) {
                const shapeArgs = node.shapeArgs, centerX = center[0], centerY = center[1], r = center[2] / 2, nodeWidth = options.nodeWidth === 'auto' ?
                    20 : options.nodeWidth, innerR = r - (0,external_highcharts_src_js_default_namespaceObject.relativeLength)(nodeWidth || 0, r), start = startAngle + factor * (shapeArgs.y || 0), end = startAngle +
