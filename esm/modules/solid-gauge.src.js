...
/** @internal */
function init(axis) {
    (0,external_highcharts_src_js_default_namespaceObject.extend)(axis, Color_ColorAxisBase);
}
/* *
 *
 *  Default export
    ...
    drawPoints() {
        ...
        for (const point of series.points) {
            ...
            if (!point.isNull) { // Condition like in pie chart
                const paneInnerSize = yAxis.pane.options.innerSize, radius = (((0,external_highcharts_src_js_default_namespaceObject.relativeLength)(point.options.radius ??
                    options.radius ??
                    '100%', center[2] / 2))), innerRadius = Math.min(((0,external_highcharts_src_js_default_namespaceObject.relativeLength)(point.options.innerRadius ??
                    options.innerRadius ??
                    ((0,external_highcharts_src_js_default_namespaceObject.isNumber)(paneInnerSize) ?
                        paneInnerSize / 2 :
                        paneInnerSize) ??
                ...
                rotation = (0,external_highcharts_src_js_default_namespaceObject.clamp)(rotation, axisMinAngle - overshootVal, axisMaxAngle + overshootVal);
                // Handle the wrap option
                if (options.wrap === false) {
                    rotation = (0,external_highcharts_src_js_default_namespaceObject.clamp)(rotation, axisMinAngle, axisMaxAngle);
                }
                ...
                if (graphic) {
                    d = shapeArgs.d;
                    graphic.animate((0,external_highcharts_src_js_default_namespaceObject.extend)(attribs, shapeArgs));
                    if (d) {
