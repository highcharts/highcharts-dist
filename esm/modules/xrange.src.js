...
const XRangeSeriesDefaults = {
    ...
    dataLabels: {
        formatter: function () {
            ...
            if ((0,external_highcharts_src_js_default_namespaceObject.isObject)(amount)) {
                amount = amount.amount;
            }
            if ((0,external_highcharts_src_js_default_namespaceObject.isNumber)(amount) && amount > 0) {
                return (0,external_highcharts_src_js_default_namespaceObject.correctFloat)(amount * 100) + '%';
            }
        },
        inside: true,
    ...
    if (this.isXAxis) {
        ...
        for (const series of this.series) {
            ...
            for (let val of column) {
                ...
                if ((0,external_highcharts_src_js_default_namespaceObject.isNumber)(val) && val > dataMax) {
                    dataMax = val;
                    modMax = true;
                }
            }
...
class XRangeSeries extends ColumnSeries {
    ...
    findPointIndex(options) {
        ...
        let pointIndex;
        if (id) {
            const point = (0,external_highcharts_src_js_default_namespaceObject.find)(points, (point) => point.id === id);
            pointIndex = point ? point.index : void 0;
        }
        if (typeof pointIndex === 'undefined') {
            const point = (0,external_highcharts_src_js_default_namespaceObject.find)(points, (point) => (point.x === options.x &&
                point.x2 === options.x2 &&
                !point.touched));
            pointIndex = point ? point.index : void 0;
        }
        ...
        if (this.cropped &&
            (0,external_highcharts_src_js_default_namespaceObject.isNumber)(pointIndex) &&
            (0,external_highcharts_src_js_default_namespaceObject.isNumber)(cropStart) &&
    ...
    translatePoint(point) {
        ...
        plotX2 = (0,external_highcharts_src_js_default_namespaceObject.clamp)(plotX2, -10, xAxis.len + 10);
        // Handle individual pointWidth
        if ((0,external_highcharts_src_js_default_namespaceObject.defined)(point.options.pointWidth)) {
            yOffset -= ((Math.ceil(point.options.pointWidth) - pointHeight) / 2);
            pointHeight = Math.ceil(point.options.pointWidth);
        }
        // Apply pointPlacement to the Y axis
        if (options.pointPlacement &&
            (0,external_highcharts_src_js_default_namespaceObject.isNumber)(point.plotY) &&
            yAxis.categories) {
            point.plotY = yAxis.translate(point.y, 0, 1, 0, 1, options.pointPlacement);
        }
        const x = (0,external_highcharts_src_js_default_namespaceObject.crisp)(Math.min(plotX, plotX2), borderWidth), x2 = (0,external_highcharts_src_js_default_namespaceObject.crisp)(Math.max(plotX, plotX2), borderWidth), width = x2 - x;
        const r = Math.min((0,external_highcharts_src_js_default_namespaceObject.relativeLength)((typeof borderRadius === 'object' ?
            borderRadius.radius :
            borderRadius || 0), pointHeight), Math.min(width, pointHeight) / 2);
        const shapeArgs = {
            x,
            y: (0,external_highcharts_src_js_default_namespaceObject.crisp)((point.plotY || 0) + yOffset, borderWidth),
            width,
            height: pointHeight,
            r
        };
        ...
        if (dlLeft < 0 || dlRight > xAxis.len) {
            dlLeft = (0,external_highcharts_src_js_default_namespaceObject.clamp)(dlLeft, 0, xAxis.len);
            dlRight = (0,external_highcharts_src_js_default_namespaceObject.clamp)(dlRight, 0, xAxis.len);
            dlWidth = dlRight - dlLeft;
            point.dlBox = (0,external_highcharts_src_js_default_namespaceObject.merge)(shapeArgs, {
                x: dlLeft,
        ...
        if (inverted) {
            ...
            tooltipPos[xIndex] = (0,external_highcharts_src_js_default_namespaceObject.clamp)(tooltipPos[xIndex] +
                (xAxis.reversed ? -1 : 0) * shapeArgs.width, xAxis.left - plotLeft, xAxis.left + xAxis.len - plotLeft - 1);
        }
        tooltipPos[yIndex] = (0,external_highcharts_src_js_default_namespaceObject.clamp)(tooltipPos[yIndex] + ((inverted ? -1 : 1) * tooltipYOffset), yAxis.top - plotTop, yAxis.top + yAxis.len - plotTop - 1);
        // Add a partShapeArgs to the point, based on the shapeArgs property
        partialFill = point.partialFill ?? 0;
        if (partialFill) {
            // Get the partial fill amount
            if ((0,external_highcharts_src_js_default_namespaceObject.isObject)(partialFill)) {
                partialFill = partialFill.amount || 0;
            }
            // If it was not a number, assume 0
            if (!(0,external_highcharts_src_js_default_namespaceObject.isNumber)(partialFill)) {
                partialFill = 0;
            }
            point.partShapeArgs = (0,external_highcharts_src_js_default_namespaceObject.merge)(shapeArgs);
    ...
    drawPoint(point, verb) {
        ...
        if (!point.isNull && point.visible !== false) {
            ...
            if (graphic) { // Update
                ...
                graphic.rect = renderer[type]((0,external_highcharts_src_js_default_namespaceObject.merge)(shapeArgs))
                    .add(graphic);
            }
            ...
            if (partShapeArgs) {
                if (graphic.partRect) {
                    graphic.partRect[verb]((0,external_highcharts_src_js_default_namespaceObject.merge)(partShapeArgs));
                    graphic.partialClipRect[verb]((0,external_highcharts_src_js_default_namespaceObject.merge)(clipRectArgs));
                }
                else {
            ...
            if (!this.chart.styledMode) {
                ...
                if (partShapeArgs) {
                    ...
                    if (!(0,external_highcharts_src_js_default_namespaceObject.isObject)(pfOptions)) {
                        pfOptions = {};
                    }
                    if ((0,external_highcharts_src_js_default_namespaceObject.isObject)(seriesOpts.partialFill)) {
                        pfOptions = (0,external_highcharts_src_js_default_namespaceObject.merge)(seriesOpts.partialFill, pfOptions);
                    }
