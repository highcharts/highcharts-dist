...
function columnAnimateDrilldown(init) {
    ...
    if (!init) {
        ...
        series.points.forEach((point) => {
            point.graphic
                ?.attr(animateFrom)
                .animate((0,external_highcharts_src_js_default_namespaceObject.extend)(point.shapeArgs, { fill: point.color || series.color }), animationOptions);
        });
    ...
    (this.points || this.data).forEach((point) => {
        ...
        if (graphic && animateTo) {
            ...
            if (animationOptions.duration) {
                graphic.animate(animateTo, (0,external_highcharts_src_js_default_namespaceObject.merge)(animationOptions, { complete: complete }));
            }
            else {
...
function compose(SeriesClass, seriesTypes) {
    ...
    if (!pointProto.doDrilldown) {
        ...
        (0,external_highcharts_src_js_default_namespaceObject.addEvent)(PointClass, 'afterInit', onPointAfterInit);
        (0,external_highcharts_src_js_default_namespaceObject.addEvent)(PointClass, 'afterSetState', onPointAfterSetState);
        (0,external_highcharts_src_js_default_namespaceObject.addEvent)(PointClass, 'update', onPointUpdate);
        pointProto.doDrilldown = pointDoDrilldown;
        pointProto.runDrilldown = pointRunDrilldown;
        (0,external_highcharts_src_js_default_namespaceObject.addEvent)(SeriesClass, 'afterDrawDataLabels', onSeriesAfterDrawDataLabels);
        (0,external_highcharts_src_js_default_namespaceObject.addEvent)(SeriesClass, 'afterDrawTracker', onSeriesAfterDrawTracker);
...
function onPointAfterInit() {
    ...
    if (point.drilldown && !point.unbindDrilldownClick) {
        ...
        point.unbindDrilldownClick = (0,external_highcharts_src_js_default_namespaceObject.addEvent)(point, 'click', onPointClick);
    }
    return point;
}
/** @internal */
...
function onSeriesAfterDrawDataLabels() {
    ...
    for (const point of series.points) {
        const dataLabelsOptions = (0,external_highcharts_src_js_default_namespaceObject.splat)(point.options.dataLabels)[0] || {}, pointCSS = (point.dlOptions ||
            dataLabelsOptions.style ||
            {});
        if (point.drilldown && point.dataLabel) {
            if (css.color === 'contrast' && !styledMode) {
                const itemColor = (((0,external_highcharts_src_js_default_namespaceObject.isString)(point.color) && point.color) ||
                    ((0,external_highcharts_src_js_default_namespaceObject.isString)(series.color) && series.color));
                if ((0,external_highcharts_src_js_default_namespaceObject.isString)(itemColor)) {
    ...
    if (series.center) {
        ...
        if (!init) {
            ...
            for (let i = 0, iEnd = points.length; i < iEnd; ++i) {
                ...
                const attr = (0,external_highcharts_src_js_default_namespaceObject.merge)(animateFrom, {
...
function axisDrilldownCategory(x, originalEvent) {
    this.getDDPoints(x).forEach(function (point) {
        if ((0,external_highcharts_src_js_default_namespaceObject.isObject)(point) &&
            point.series &&
        ...
        drilldownLevels.forEach(function (level) {
            ...
            if (level.levelNumber + 1 > lastBreadcrumb.level) {
                list.push({
                    ...
                    levelOptions: (0,external_highcharts_src_js_default_namespaceObject.merge)({
...
class ChartAdditions {
    ...
    addSeriesAsDrilldown(point, options) {
        const chart = (this.chart ||
            this);
        (0,external_highcharts_src_js_default_namespaceObject.fireEvent)(this, 'addSeriesAsDrilldown', { seriesOptions: options });
        if (chart.mapView) {
            ...
            if (chart.options.drilldown &&
                ...
                const userDrilldown = (0,external_highcharts_src_js_default_namespaceObject.diffObjects)(chart.options.drilldown, Drilldown_DrilldownDefaults);
                // Set mapZooming to false if user didn't set any in chart
                // config
                if (!(0,external_highcharts_src_js_default_namespaceObject.defined)(userDrilldown.mapZooming)) {
    ...
    addSingleSeriesAsDrilldown(point, ddOptions) {
        ...
        ddOptions = (0,external_highcharts_src_js_default_namespaceObject.extend)((0,external_highcharts_src_js_default_namespaceObject.extend)({
        ...
        oldSeries.chart.series.forEach((series) => {
            ...
            if (series.xAxis === xAxis) {
                ...
                if (last) {
                    ...
                    // (#10597)
                    series.purgedOptions = (0,external_highcharts_src_js_default_namespaceObject.merge)({
        ...
        const level = (0,external_highcharts_src_js_default_namespaceObject.extend)({
                        ...
                        if (!chart.mapView) {
                            ...
                            const removeSeries = () => {
                                ...
                                if (!(level.levelSeries.filter((el) => Object.keys(el).length)).length) {
                                    ...
                                    chart.pointer?.reset();
                                    (0,external_highcharts_src_js_default_namespaceObject.fireEvent)(chart, 'afterDrilldown');
                                    if (chart.mapView) {
                                        chart.series.forEach((series) => {
                            ...
                            if (series.group?.element) {
                                ...
                                (0,external_highcharts_src_js_default_namespaceObject.syncTimeout)(removeSeries, drillAnimOptions.defer +
    ...
    drillUp(isMultipleDrillUp) {
        ...
        while (i--) {
            ...
            if (level.levelNumber === levelNumber) {
                ...
                (0,external_highcharts_src_js_default_namespaceObject.fireEvent)(chart, 'drillup', {
                ...
                if (!chart.mapView) {
                    (0,external_highcharts_src_js_default_namespaceObject.fireEvent)(chart, 'afterDrillUp');
                }
                else {
    ...
    update(options, redraw = true) {
        const chart = this.chart;
        (0,external_highcharts_src_js_default_namespaceObject.merge)(true, chart.options.drilldown, options);
        if (redraw) {
            chart.redraw();
        }
    }
...
(function (Drilldown) {
    ...
    function compose(AxisClass, ChartClass, highchartsDefaultOptions, SeriesClass, seriesTypes, SVGRendererClass, TickClass) {
        ...
        if (!chartProto.drillUp) {
            ...
            (0,external_highcharts_src_js_default_namespaceObject.addEvent)(Breadcrumbs_Breadcrumbs, 'up', onBreadcrumbsUp);
            ...
            (0,external_highcharts_src_js_default_namespaceObject.addEvent)(DrilldownChart, 'afterDrilldown', onChartAfterDrilldown);
            (0,external_highcharts_src_js_default_namespaceObject.addEvent)(DrilldownChart, 'afterDrillUp', onChartAfterDrillUp);
            (0,external_highcharts_src_js_default_namespaceObject.addEvent)(DrilldownChart, 'afterInit', onChartAfterInit);
            (0,external_highcharts_src_js_default_namespaceObject.addEvent)(DrilldownChart, 'drillup', onChartDrillup);
            (0,external_highcharts_src_js_default_namespaceObject.addEvent)(DrilldownChart, 'drillupall', onChartDrillupall);
            (0,external_highcharts_src_js_default_namespaceObject.addEvent)(DrilldownChart, 'render', onChartRender);
            (0,external_highcharts_src_js_default_namespaceObject.addEvent)(DrilldownChart, 'update', onChartUpdate);
            (0,external_highcharts_src_js_default_namespaceObject.addEvent)(SeriesClass, 'update', onSeriesUpdate);
        ...
        (this.xAxis || []).forEach((axis) => {
            ...
            axis.series.forEach((series) => {
                ...
                for (let i = 0, iEnd = xData.length, p; i < iEnd; i++) {
                    ...
                    // object
                    if ((0,external_highcharts_src_js_default_namespaceObject.defined)(p) && typeof p !== 'number') {
            ...
            // updated (#3951)
            (0,external_highcharts_src_js_default_namespaceObject.objectEach)(axis.ticks, (tick) => tick.drillable());
        });
    }
    /** @internal */
        ...
        if (isDrillable) {
            if (label && ddPointsX && ddPointsX.length) {
                ...
                if (!label.basicStyles && !styledMode) {
                    label.basicStyles = (0,external_highcharts_src_js_default_namespaceObject.merge)(label.styles);
                }
                label.addClass('highcharts-drilldown-axis-label');
                // #12656 - avoid duplicate of attach event
                if (label.removeOnDrillableClick) {
                    (0,external_highcharts_src_js_default_namespaceObject.removeEvent)(label.element, 'click');
                }
                label.removeOnDrillableClick = (0,external_highcharts_src_js_default_namespaceObject.addEvent)(label.element, 'click', function (e) {
                    e.preventDefault();
