...
const { composed: PointAndFigureSeries_composed } = (external_highcharts_src_js_default_default());
...
class PointAndFigureSeries extends ScatterSeries {
    ...
    getProcessedData() {
        ...
        const series = this, options = series.options, xData = series.getColumn('x', true), yData = series.getColumn('y', true), boxSize = options.boxSize, calculatedBoxSize = (0,external_highcharts_src_js_default_namespaceObject.isNumber)(boxSize) ?
            boxSize : (0,external_highcharts_src_js_default_namespaceObject.relativeLength)(boxSize, yData[0]), pnfDataGroups = series.pnfDataGroups, reversal = calculatedBoxSize * options.reversalAmount, dataTable = this.dataTable.getModified(), modified = dataTable === this.dataTable ?
            dataTable :
