    ...
    renderCollapseButton() {
        ...
        if (!point.collapseButton) {
            ...
            point.collapseButton = chart.renderer
                ...
                .css((0,external_highcharts_src_js_default_namespaceObject.merge)({
...
(0,external_highcharts_src_js_default_namespaceObject.addEvent)(TreegraphPoint, 'mouseOver', function () {
...
(0,external_highcharts_src_js_default_namespaceObject.addEvent)(TreegraphPoint, 'click', function () {
    this.toggleCollapse();
});
/* *
 *
...
function composeTextPath(SVGElementClass) {
    if ((0,external_highcharts_src_js_default_namespaceObject.pushUnique)(composed, 'TextPath')) {
...
class TreegraphSeries extends TreemapSeries {
    ...
    init() {
        ...
        const series = this, collectors = this.chart.labelCollectors, collectorFunc = function () {
            ...
            if (series.options.dataLabels &&
                !(0,external_highcharts_src_js_default_namespaceObject.splat)(series.options.dataLabels)[0].allowOverlap) {
     ...
     */
    getLayoutModifiers() {
        const chart = this.chart, series = this, plotSizeX = chart.plotSizeX, plotSizeY = chart.plotSizeY, linkWidth = series.options.link?.lineWidth || 0, columnCount = (0,external_highcharts_src_js_default_namespaceObject.arrayMax)(this.points.map((p) => p.node.xPosition));
        ...
        this.points.forEach((point) => {
            ...
            const node = point.node, level = series.mapOptionsToLevel[point.node.level] || {}, markerOptions = (0,external_highcharts_src_js_default_namespaceObject.merge)(this.options.marker, level.marker, point.options.marker), nodeWidth = markerOptions.width ?? TreegraphSeries_getNodeWidth(this, columnCount), radius = (0,external_highcharts_src_js_default_namespaceObject.relativeLength)(markerOptions.radius || 0, Math.min(plotSizeX, plotSizeY)), lineWidth = Math.max(markerOptions.lineWidth || 0, linkWidth), symbol = markerOptions.symbol, nodeSizeY = (symbol === 'circle' || !markerOptions.height) ?
                radius * 2 :
                (0,external_highcharts_src_js_default_namespaceObject.relativeLength)(markerOptions.height, plotSizeY), nodeSizeX = symbol === 'circle' || !nodeWidth ?
                radius * 2 :
                (0,external_highcharts_src_js_default_namespaceObject.relativeLength)(nodeWidth, plotSizeX);
    ...
    getLinks() {
        ...
        this.data.forEach((point) => {
            ...
            if (point.node.parent) {
                const pointOptions = (0,external_highcharts_src_js_default_namespaceObject.merge)(levelOptions, point.options);
                if (!point.linkToParent || point.linkToParent.condemned) {
                    ...
                    // #19552
                    point.collapsed = (0,external_highcharts_src_js_default_namespaceObject.pick)(point.collapsed, (this.mapOptionsToLevel[point.node.level] || {}).collapsed);
    ...
    setCollapsedStatus(node, visibility) {
        ...
        if (point) {
            ...
            point.collapsed = (0,external_highcharts_src_js_default_namespaceObject.pick)(point.collapsed, (this.mapOptionsToLevel[node.level] || {}).collapsed);
    ...
    translateLink(link) {
        const fromNode = link.fromNode, toNode = link.toNode, linkWidth = this.options.link?.lineWidth || 0, factor = (0,external_highcharts_src_js_default_namespaceObject.pick)(this.options.link?.curveFactor, 0.5), hasXData = toNode.x !== toNode.node.level ||
            fromNode.x !== fromNode.node.level, type = (0,external_highcharts_src_js_default_namespaceObject.pick)(link.options.link?.type, this.options.link?.type, 'default');
        if (fromNode.shapeArgs && toNode.shapeArgs) {
            const fromNodeWidth = (fromNode.shapeArgs.width || 0), inverted = this.chart.inverted, y1 = (0,external_highcharts_src_js_default_namespaceObject.crisp)((fromNode.shapeArgs.y || 0) +
                (fromNode.shapeArgs.height || 0) / 2, linkWidth), y2 = (0,external_highcharts_src_js_default_namespaceObject.crisp)((toNode.shapeArgs.y || 0) +
                (toNode.shapeArgs.height || 0) / 2, linkWidth);
            let x1 = (0,external_highcharts_src_js_default_namespaceObject.crisp)((fromNode.shapeArgs.x || 0) + fromNodeWidth, linkWidth), x2 = (0,external_highcharts_src_js_default_namespaceObject.crisp)(toNode.shapeArgs.x || 0, linkWidth);
            if (inverted) {
                x1 -= Math.round(fromNodeWidth);
                x2 += Math.round(toNode.shapeArgs.width || 0);
            }
            const xDiff = toNode.node.xPosition - fromNode.node.xPosition, fullWidth = Math.abs(x2 - x1) + fromNodeWidth, scopeWidth = hasXData ? fullWidth : (fullWidth / xDiff), width = scopeWidth - fromNodeWidth, offset = width * factor * (inverted ? -1 : 1), xMiddle = (0,external_highcharts_src_js_default_namespaceObject.crisp)((x2 + x1) / 2, linkWidth), bendAt = (0,external_highcharts_src_js_default_namespaceObject.relativeLength)(link.options.link?.bendAt ??
    ...
    drawNodeLabels(points) {
        ...
        for (const point of points) {
            ...
            if (level && level.dataLabels) {
                options = (0,external_highcharts_src_js_default_namespaceObject.merge)(options, level.dataLabels);
            ...
            if (point.shapeArgs &&
                ...
                if (!(0,external_highcharts_src_js_default_namespaceObject.splat)(series.options.dataLabels)[0].style?.width) {
                    css.width = `${width}px`;
                }
    ...
    drawDataLabels() {
        if (this.options.dataLabels) {
            this.options.dataLabels = (0,external_highcharts_src_js_default_namespaceObject.splat)(this.options.dataLabels);
            // Render node labels.
    ...
    pointAttribs(point, state) {
        const series = this, levelOptions = point &&
            ...
            {};
        if (point) {
            point.options.marker = (0,external_highcharts_src_js_default_namespaceObject.merge)(series.options.marker, levelOptions.marker, point.options.marker);
        }
        const linkColor = (0,external_highcharts_src_js_default_namespaceObject.pick)(stateOptions && stateOptions.link && stateOptions.link.color, options && options.link && options.link.color, levelOptions && levelOptions.link && levelOptions.link.color, series.options.link && series.options.link.color), linkLineWidth = (0,external_highcharts_src_js_default_namespaceObject.pick)(stateOptions && stateOptions.link &&
    ...
    translateNode(point) {
        ...
        { ax, bx, ay, by } = this.layoutModifier, x = this.isCartesian ?
            ...
            ax * node.xPosition + bx, y = ay * node.yPosition + by, level = this.mapOptionsToLevel[node.level] || {}, markerOptions = (0,external_highcharts_src_js_default_namespaceObject.merge)(this.options.marker, level.marker, point.options.marker), symbol = markerOptions.symbol, height = node.nodeSizeY, width = node.nodeSizeX, reversed = this.options.reversed, nodeX = node.x = (chart.inverted ?
            plotSizeX - width / 2 - x :
            x - width / 2), nodeY = node.y = (!reversed ?
            plotSizeY - y - height / 2 :
            y - height / 2), borderRadius = (0,external_highcharts_src_js_default_namespaceObject.pick)(point.options.borderRadius, level.borderRadius, this.options.borderRadius), symbolFn = symbols[symbol || 'circle'];
        if (symbolFn === void 0) {
            point.hasImage = true;
        ...
        if (!point.visible && point.linkToParent) {
            ...
            if (parentNode) {
                ...
                if (!point.hasImage) {
                    (0,external_highcharts_src_js_default_namespaceObject.extend)(point.shapeArgs, {
