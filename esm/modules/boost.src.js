...
class WGLShader {
    ...
    handleErrors() {
        if (this.errors.length) {
            (0,external_highcharts_src_js_default_namespaceObject.error)('[highcharts boost] shader error - ' +
                this.errors.join('\n'));
        }
    }
    /**
...
function createAndAttachRenderer(chart, series) {
    ...
    if (!boost.wgl) {
        ...
        if (!boost.wgl.init(boost.canvas)) {
            ...
            // support.
            (0,external_highcharts_src_js_default_namespaceObject.error)('[highcharts boost] - unable to init WebGL renderer');
        }
...
function Boost_compose(ChartClass, AxisClass, SeriesClass, seriesTypes, PointClass, ColorClass) {
    ...
    if (!wglMode) {
        if (typeof (external_highcharts_src_js_default_default()).initCanvasBoost !== 'undefined') {
            ...
            (0,external_highcharts_src_js_default_namespaceObject.error)(26);
        }
    }
    ...
    // div while `isPanning`.
    (0,external_highcharts_src_js_default_namespaceObject.addEvent)(AxisClass, 'setExtremes', function (e) {
