...
function addEvents(el, types, fn, options) {
    const removeFuncs = types.map((type) => (0,external_highcharts_src_js_default_namespaceObject.addEvent)(el, type, fn, options));
    return function () {
...
function addDragDropEvents(chart) {
    ...
    if (isChartDraggable(chart)) {
        ...
        (0,external_highcharts_src_js_default_namespaceObject.addEvent)(container, 'mouseleave', (e) => {
        ...
        (0,external_highcharts_src_js_default_namespaceObject.addEvent)(chart, 'destroy', () => {
...
function chartSetGuideBoxState(state, options) {
    const guideBox = this.dragGuideBox, guideBoxOptions = (0,external_highcharts_src_js_default_namespaceObject.merge)(DraggablePoints_DragDropDefaults.guideBox, options), stateOptions = (0,external_highcharts_src_js_default_namespaceObject.merge)(guideBoxOptions['default'], // eslint-disable-line dot-notation
...
function dragMove(e, point) {
    const series = point.series, chart = series.chart, data = chart.dragDropData, options = (0,external_highcharts_src_js_default_namespaceObject.merge)(series.options.dragDrop, point.options.dragDrop), draggableX = options.draggableX, draggableY = options.draggableY, origin = data.origin, updateProp = data.updateProp;
...
function getGroupedPoints(point) {
    ...
    let points = [];
    if (series.boosted && (0,external_highcharts_src_js_default_namespaceObject.isArray)(data)) { // #11156
...
function getNewPoints(dragDropData, newPos) {
    const point = dragDropData.point, series = point.series, chart = series.chart, options = (0,external_highcharts_src_js_default_namespaceObject.merge)(series.options.dragDrop, point.options.dragDrop), updateProps = {}, resizeProp = dragDropData.updateProp, hashmap = {}, dragDropProps = point.series.dragDropProps;
...
function mouseDown(e, chart) {
    const dragPoint = chart.hoverPoint, dragDropOptions = (0,external_highcharts_src_js_default_namespaceObject.merge)(dragPoint && dragPoint.series.options.dragDrop, dragPoint && dragPoint.options.dragDrop), draggableX = dragDropOptions.draggableX || false, draggableY = dragDropOptions.draggableY || false;
    // Reset cancel click
...
function updatePoints(chart, animation) {
    const newPoints = chart.dragDropData.newPoints, animOptions = (0,external_highcharts_src_js_default_namespaceObject.animObject)(animation);
...
const column = {
    ...
    y: {
        ...
        beforeResize: (guideBox, pointVals, point) => {
            ...
            const plotThreshold = (0,external_highcharts_src_js_default_namespaceObject.pick)(point.yBottom, // Added support for stacked series. (#18741)
            point.series.translatedThreshold), plotY = guideBox.attr('y'), threshold = (0,external_highcharts_src_js_default_namespaceObject.isNumber)(point.stackY) ? (point.stackY - (point.y || 0)) : point.series.options.threshold || 0, y = threshold + pointVals.y;
            let height, diff;
...
const waterfall = {
    x: column.x,
    y: (0,external_highcharts_src_js_default_namespaceObject.merge)(column.y, {
...
const gantt = {
     ...
     */
    start: (0,external_highcharts_src_js_default_namespaceObject.merge)(xrange.x, {
     ...
     */
    end: (0,external_highcharts_src_js_default_namespaceObject.merge)(xrange.x2, {
...
function DraggablePoints_compose(ChartClass, SeriesClass) {
    ...
    if (!seriesProto.dragDropProps) {
        ...
        (0,external_highcharts_src_js_default_namespaceObject.addEvent)(PointClass, 'mouseOut', onPointMouseOut);
        (0,external_highcharts_src_js_default_namespaceObject.addEvent)(PointClass, 'mouseOver', onPointMouseOver);
        (0,external_highcharts_src_js_default_namespaceObject.addEvent)(PointClass, 'remove', onPointRemove);
...
function pointGetDropValues(origin, newPos, updateProps) {
    const point = this, series = point.series, chart = series.chart, mapView = chart.mapView, options = (0,external_highcharts_src_js_default_namespaceObject.merge)(series.options.dragDrop, point.options.dragDrop), result = {}, pointOrigin = origin.points[point.id], updateSingleProp = Object.keys(updateProps).length === 1;
    /**
    ...
    const limitToRange = (val, dir) => {
        ...
        return (0,external_highcharts_src_js_default_namespaceObject.clamp)(res, min, max);
    };
    /**
     * Utility function to apply precision and limit a value within the
    ...
    for (const key of Object.keys(updateProps)) {
        ...
        if ((0,external_highcharts_src_js_default_namespaceObject.isNumber)(newVal) &&
            !(updateSingleProp &&
                val.propValidate &&
...
function pointShowDragHandles() {
    const point = this, series = point.series, chart = series.chart, { inverted, renderer } = chart, options = (0,external_highcharts_src_js_default_namespaceObject.merge)(series.options.dragDrop, point.options.dragDrop), dragDropProps = series.dragDropProps || {};
    let dragHandles = chart.dragHandles;
    // Go through each updateProp and see if we are supposed to create a handle
    // for it.
    for (const key of Object.keys(dragDropProps)) {
        const val = dragDropProps[key], handleOptions = (0,external_highcharts_src_js_default_namespaceObject.merge)(DraggablePoints_DragDropDefaults.dragHandle, val.handleOptions, options.dragHandle), handleAttrs = {
        ...
        if (val.resize &&
            ...
            (0,external_highcharts_src_js_default_namespaceObject.addEvent)(dragHandles.group.element, 'mouseover', () => {
...
function seriesGetGuideBox(points) {
    ...
    for (const point of points) {
        ...
        if (bBox) {
            let plotX2;
            const x2 = point.x2;
            if ((0,external_highcharts_src_js_default_namespaceObject.isNumber)(x2)) {
