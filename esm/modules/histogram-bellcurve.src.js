...
(function (DerivedComposition) {
    ...
    function addBaseSeriesEvents() {
        this.eventRemovers.push((0,external_highcharts_src_js_default_namespaceObject.addEvent)(this.baseSeries, 'updatedData', () => {
            this.setDerivedData();
        }), (0,external_highcharts_src_js_default_namespaceObject.addEvent)(this.baseSeries, 'destroy', () => {
            this.baseSeries = null;
            this.initialised = false;
        }));
    }
...
class HistogramSeries extends ColumnSeries {
    ...
    binsNumber(data) {
        ...
        return Math.ceil((binsNumber && binsNumber(data)) ||
            ((0,external_highcharts_src_js_default_namespaceObject.isNumber)(binsNumberOption) ?
                binsNumberOption :
    ...
    setData(data, redraw = true, animation, updatePoints) {
        ...
        if (typeof data !== 'undefined' && data.length > 0) {
            ...
            data = data.map(function (item) {
                return (0,external_highcharts_src_js_default_namespaceObject.isNumber)(item) ? item : item?.y ?? 0;
            });
            alteredData = this.derivedData(data.filter(external_highcharts_src_js_default_namespaceObject.isNumber), this.binsNumber(data), this.options.binWidth);
        }
    ...
    derivedData(baseData, binsNumber, binWidth) {
        ...
        min = (0,external_highcharts_src_js_default_namespaceObject.correctFloat)((0,external_highcharts_src_js_default_namespaceObject.arrayMin)(baseData)), frequencies = [], bins = {}, data = [];
        let x;
        binWidth = series.binWidth = ((0,external_highcharts_src_js_default_namespaceObject.correctFloat)((0,external_highcharts_src_js_default_namespaceObject.isNumber)(binWidth) ?
            (binWidth || 1) :
            (max - min) / binsNumber));
        ...
        x < max &&
            (series.userOptions.binWidth ||
                (0,external_highcharts_src_js_default_namespaceObject.correctFloat)(max - x) >= binWidth ||
                // #13069 - Every add and subtract operation should
                // be corrected, due to general problems with
                // operations on float numbers in JS.
                (0,external_highcharts_src_js_default_namespaceObject.correctFloat)((0,external_highcharts_src_js_default_namespaceObject.correctFloat)(min + (frequencies.length * binWidth)) -
                    x) <= 0); x = (0,external_highcharts_src_js_default_namespaceObject.correctFloat)(x + binWidth)) {
            frequencies.push(x);
            bins[x] = 0;
        }
        if (bins[min] !== 0) {
        ...
        for (const y of baseData) {
            bins[(0,external_highcharts_src_js_default_namespaceObject.correctFloat)(fitToBin(y))]++;
        }
        for (const key of Object.keys(bins)) {
            data.push({
                x: Number(key),
                y: bins[key],
                x2: (0,external_highcharts_src_js_default_namespaceObject.correctFloat)(Number(key) + binWidth)
            });
        }
...
class BellcurveSeries extends AreaSplineSeries {
    ...
    static standardDeviation(data, average) {
        ...
        average = (0,external_highcharts_src_js_default_namespaceObject.isNumber)(average) ?
    ...
    setData(data, redraw = true, animation, updatePoints) {
        ...
        if (typeof data !== 'undefined' && data.length > 0) {
            ...
            data = data
                .map(function (item) {
                return (0,external_highcharts_src_js_default_namespaceObject.isNumber)(item) ? item : item?.y;
            })
